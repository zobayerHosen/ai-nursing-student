...
// Generate extra toons to fill the 16 topics slot by mapping other subcategories to these 3 structures
export const getToonBySlug = (slug) => {
  // Direct matches
  const direct = toonsData.find(t => t.slug === slug || t.topicSlug === slug);
  if (direct) return direct;

  // Fallbacks based on category/subcategories
  ...
  if (["abruptio-placentae", "transcultural-care"].includes(slug)) {
    return {
      ...
      slug: slug,
      topicSlug: slug,
      title: slug === "abruptio-placentae" ? "Abruptio Placentae: High-Risk Alert" : toonsData[1].title,
      category: slug === "abruptio-placentae" ? "OB - High-Risk" : toonsData[1].category,
    };
  }
  if (["nms", "depression-anxiety", "immunization-schedules", "evidence-hierarchy"].includes(slug)) {
    return {
      ...
      slug: slug,
      topicSlug: slug,
      title: slug === "nms" ? "Neuroleptic Malignant Syndrome (NMS)" : toonsData[2].title,
    };
  }
...
// Generate 16 items grid by repeating the 3 primary toon types with customized ids
export const getToonsGrid = () => {
  const result = [];
  const titles = [
  ...
  for (let i = 0; i < 16; i++) {
    const baseToon = toonsData[i % 3];
    result.push({
      ...baseToon,
      id: i + 1,
      slug: `${baseToon.slug}-${i + 1}`,
