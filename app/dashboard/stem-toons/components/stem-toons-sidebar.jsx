...
const SidebarContent = ({ onClose }) => {
  ...
  const { coreLearningData, isLoading, coreLearningPagination, isFetching } = useCoreLearning("stem_toons", { limit });
                  ...
                  <div className="overflow-hidden">
                    <div className="space-y-2">
                      {category?.contents?.map(
                        (subcategory, index) => {
                          const isActive = pathname.includes(`/stem-toons/${subcategory?.id}`);
                          return (
                            <Link
                              href={`/dashboard/stem-toons/${subcategory?.id}`}
                              key={index}
                              onClick={() => {
                                if (onClose) onClose();
                              }}
...
const StemToonsSidebar = ({ onClose }) => {
  return (
