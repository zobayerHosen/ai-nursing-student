...
import StemBreadcrumb from "./components/stem-breadcrumb";

export default function ToonDetailsPage() {
  const { toonslug } = useParams();
  const contentId = Number(toonslug);
        ...
        <div className="bg-white rounded-2xl p-10 border border-[#EEEEEE] shadow-sm text-center">
          <div className="max-w-md mx-auto">
            ...
            <p className="text-[#7A7A7A] mb-6">
              The Stem Toon you are looking for does not exist or has been
              moved.
            </p>
            <Link
  ...
  return (
    <div className="w-full flex flex-col">
      {/* Top Buttons */}
      <StemBreadcrumb currentCategory={currentCategory} currentNote={currentNote} />
        ...
        <div className="w-full h-[calc(100vh-250px)] min-h-[500px] relative">
          {currentNote?.content_file_url ? (
            ...
            <div className="flex items-center justify-center h-full text-gray-500">
              No content available for this toon.
            </div>
          )}
        </div>
      </div>
