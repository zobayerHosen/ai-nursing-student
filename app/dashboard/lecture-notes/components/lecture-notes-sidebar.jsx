    ...
    <aside className="w-[320px] bg-white h-full flex flex-col p-6 shrink-0 border-r border-gray-200 shadow-lg lg:shadow-none overflow-y-auto">
      ...
      <div className="relative flex border-b border-gray-200 mb-6">
        <button
          ...
          className={`relative flex flex-1 items-center justify-center gap-1.5 px-4 py-2.5 text-sm font-semibold transition ${
            activeTab === 'upload' ? 'text-[#2B5C8F]' : 'text-gray-400 hover:text-[#2B5C8F]'
          }`}
        >
          ...
          {activeTab === 'upload' && (
            <motion.div
              ...
              className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#2B5C8F]"
        ...
        <button
          ...
          className={`relative flex flex-1 items-center justify-center gap-1.5 px-4 py-2.5 text-sm font-semibold transition ${
            activeTab === 'notes' ? 'text-[#2B5C8F]' : 'text-gray-400 hover:text-[#2B5C8F]'
          }`}
        >
          ...
          {notesList.length > 0 && (
            <span
              className={`ml-0.5 inline-flex items-center justify-center min-w-[18px] h-[18px] rounded-full px-1.5 text-[10px] font-bold leading-none ${
                activeTab === 'notes' ? 'bg-[#2B5C8F] text-white' : 'bg-[#E2E8F0] text-[#475569]'
              }`}
            >
              {notesList.length > 99 ? '99+' : notesList.length}
            </span>
          )}
          {activeTab === 'notes' && (
            <motion.div
