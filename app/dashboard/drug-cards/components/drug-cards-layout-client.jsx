...
import DrugCardsSidebar from "./drug-cards-sidebar";

export default function DrugCardsLayoutClient({ children }) {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);

  return (
    <div className="flex flex-col xl:flex-row h-full xl:min-h-[calc(100vh-80px)] relative w-full">
      ...
      >
        <DrugCardsSidebar onClose={() => setIsSidebarOpen(false)} />
      </div>
      ...
      <main className="flex-1 transition-all duration-300 overflow-y-auto w-full bg-[#F8F9FA]">
        ...
        <div className="xl:hidden p-4 border-b border-black/10 flex items-center gap-3 bg-white sticky top-0 z-30">
          ...
          <h2 className="font-semibold text-lg text-[#2C5F8D]">Drug Cards</h2>
        </div>

        <div className="p-4 xl:p-6 w-full max-w-7xl mx-auto">{children}</div>
      </main>
    </div>
  );
}
