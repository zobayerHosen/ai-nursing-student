...
/**
 * Reusable Dashboard Breadcrumb used across all learning modules.
 *
 * Props:
 *  - backHref      {string}  URL for the parent page link (required)
 *  - backLabel     {string}  Text for the parent link, falls back to currentCategory?.title (required)
 *  - currentLabel  {string}  Current page label shown as the last crumb (defaults to currentNote?.content_name)
 *  - topicCount    {number}  Topic count shown next to the parent label (optional)
 *  - saveLabel     {string}  Label for the save button, e.g. "Save Note" / "Save Toon" (default: "Save")
 *  - showComplete  {boolean} Whether to show the Mark Completed button (default: true)
 *  - currentNote   {object}  The note/content detail object (id, is_saved, is_completed, content_name, …)
 *  - currentCategory {object} Optional category object with title & contents
 */
const DashboardBreadcrumb = ({
    ...
    const parentLabel = backLabel || currentCategory?.title || "Dashboard";
    const count = topicCount ?? currentCategory?.contents?.length ?? null;
    const leafLabel = currentLabel || currentNote?.content_name;
        ...
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-6">
            ...
            <nav className="overflow-x-auto whitespace-nowrap pb-2 xl:pb-0 hide-scrollbar">
                <ol className="flex items-center text-sm xl:text-base">
                    ...
                    <li>
                        <span className="text-[#7A7A7A]">{leafLabel}</span>
                    </li>
                </ol>
            </nav>
