...
import VideoLessonsSidebar from "./components/video-lessons-sidebar";
...
export default function VideoLessonsLayout({ children }) {
  ...
  return (
    <div className="flex flex-col xl:flex-row h-full xl:min-h-[calc(100vh-80px)] relative">
      ...
      >
        <VideoLessonsSidebar onClose={() => setIsSidebarOpen(false)} />
      </div>
      ...
      <main className="flex-1 transition-all duration-300 overflow-y-auto w-full">
        ...
        <div className="xl:hidden p-4 border-b border-black/10 flex items-center gap-3 bg-white sticky top-0 z-30">
          ...
          <h2 className="font-semibold text-lg text-[#1B4B66]">
            Video Lessons Menu
          </h2>
        </div>
