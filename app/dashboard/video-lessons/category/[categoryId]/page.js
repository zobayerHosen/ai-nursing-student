...
import dummayImage from "@/public/med_dumm.jpg";
...
function CategoryDetailContent({ params: propParams, topic_id }) {
    const router = useRouter();
    const searchParams = useSearchParams();
    const from = searchParams.get("from");

    const routeParams = useParams();
    const categoryId = topic_id || routeParams?.categoryId || propParams?.categoryId;
                        ...
                        {filteredCategoryVideos?.map((video) => (
                            ...
                            >
                                ...
                                <div className="relative aspect-video w-full bg-slate-900 overflow-hidden">
                                    ...
                                    {/* Duration Badge */}
                                    {video.duration_formatted && video.duration_formatted !== "0m" ? (
                                        <div className="absolute bottom-2 right-2 bg-black/75 backdrop-blur-xs text-white text-[10px] font-semibold px-2 py-0.5 rounded-md">
                                    ...
                                    {(video.progress_percent > 0 ||
                                        video.user_progress?.progress_percent > 0) &&
                                        !video.is_completed &&
                                        !video.user_progress?.is_completed && (
                                            <div className="absolute inset-x-0 bottom-0 h-1 bg-black/40">
                                                <div
                                                    className="h-full bg-[#e14564]"
                                                    style={{
                                                        width: `${Math.min(
                                                            100,
                                                            video.progress_percent ||
                                                            video.user_progress?.progress_percent ||
                                                            0
                                                        )}%`,
                                                    }}
                                                />
